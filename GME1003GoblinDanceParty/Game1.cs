...
namespace GME1003GoblinDanceParty
{
    public class Game1 : Game
    {
        ...
        protected override void Initialize()
        {
            ...
            for (int i = 0; i < _numStars; i++) 
            { 
                Color starColor = new Color(128 + _rng.Next(0, 129), 128 + _rng.Next(0, 129), 128 + _rng.Next(0, 129));
                _starColor.Add(starColor);
            }
            //ToDo: List of scale values
            for (int i =0; i < _numStars; i++) 
            {
                float starScale = _rng.Next(50, 100) / 200f;
                _starScale.Add(starScale);
            }
